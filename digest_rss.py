...
# Быстрая самопроверка формата секретов (не выводит сами значения в лог)
if ":" not in TELEGRAM_BOT_TOKEN:
    print("ПРЕДУПРЕЖДЕНИЕ: TELEGRAM_BOT_TOKEN не похож на настоящий токен бота "
          "(в нём должно быть двоеточие вида 123456789:AA...). Проверьте секрет в GitHub.")
if not TELEGRAM_CHAT_ID.lstrip("-").isdigit():
    print("ПРЕДУПРЕЖДЕНИЕ: TELEGRAM_CHAT_ID должен состоять только из цифр "
          "(и, возможно, минуса в начале для групповых чатов). Проверьте секрет в GitHub.")
...
def send_to_telegram(text: str) -> None:
    ...
    for chunk in chunks:
        ...
        if not resp.ok:
            print(f"Первая попытка (с Markdown) не удалась: {resp.status_code} {resp.text}")
            ...
            if not resp2.ok:
                print(f"Вторая попытка (без Markdown) тоже не удалась: {resp2.status_code} {resp2.text}")
